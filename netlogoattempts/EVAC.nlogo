...
We can then let these dogs herd sheep 50 times and look at the results versus an unevolved randomly intitialised set of dogs. We can also compare the behaviour against a simple algorithm that picks a direction for the dogs to move at random on each tick. 

ie. for 50 runs of evolving solutions for 20 generations. On each generation a score/fitness is recorded, at the end of the 20 generations we calculate the mean, median and minimum values. 

We can also examine the distribution of the scores of the dogs over these 50 runs.
