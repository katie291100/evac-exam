...
to endOfCycle
  ...
  let sumFitness 0
  ask dogs [set points (sum map [x -> [newVariance] of x - [prevVariance] of x ] influencedSheep)/ length influencedSheep]
  ...
  [
    ...
    [
      ...
      [
        ...
        if rank < (random-float 1 * population)
        [
...
to action
  ...
  ifelse patch-here = patch meanX meanY [
    ...
    ifelse count sheeps in-cone distanceFromMean 90 > 1 [
      ...
      ifelse count sheeps in-cone distanceFromMean 300 > 1[
        set color pink
        set dirCentre 0
