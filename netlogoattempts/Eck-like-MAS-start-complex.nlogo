...
;  set no-sheepCentre 3
;  set no-dirCentre
  set no-agent-states 15
  set no-agent-views 3
...
to endOfCycle
  ...
  ask dogs [
    ...
    ifelse patch-here = patch meanX meanY [
      ...
      set num count sheeps in-cone distanceFromMean 90
    ]
    set points (checkFitness influencedSheep meanX meanY) - num
    set sumFitness (sumFitness + points  )]

  set averageFitness (sumFitness / population)

  ; RANKING

  let rankings sort-on [points] dogs
  print map [x -> [points] of x] rankings
  ask dogs [
    set rank position self rankings
  ]
...
to-report mutate-chromosome
  ...
  let stateBlock chromosome
    while [j < (no-agent-views * no-agent-states)]
    [
...
to tick-sheeps
  ...
  [
    ...
    set nextMove calculate-direction patch-here nextPatch
...
;    set dirCentre 0
;  ][
;  ifelse angleToMean <= 45 or angleToMean > 315 [
;    set pcolor red
;    set dirCentre 1
;  ][
;  ifelse angleToMean <= 135 and angleToMean > 45 [
;    set pcolor blue
;    set dirCentre 2
;  ][
;  ifelse angleToMean <= 225 and angleToMean > 135 [
;    set pcolor yellow
;    set dirCentre 3
;  ][
;  ifelse angleToMean <= 315 and angleToMean > 225 [
;      set pcolor pink
;    set dirCentre 4
;  ][
;          print "OH NO"
...
;
  ...
  ; (i - 1) * (max_j * max_k) + (j - 1) * max_k + (k - 1)
;  let state item ((dirCentre * 3 * 16) + (sheepCentre * 16) + currentState) chromosome
...
;
  ...
  if moveDir = 0 [
    set heading 0
;    print filter [x -> item 0 x = 0] chromosome
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot averageFitness"
...
167
38
THIS CODE IS NEEDS TO BE TESTED NETLOGO 6.4
11
0.0
1
