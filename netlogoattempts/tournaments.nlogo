...
to endOfCycle
  ...
  ask sheeps [set prevVariance newVariance]
  let meanX mean [pxcor] of sheeps
  let meanY mean [pycor] of sheeps
  ask sheeps [set newVariance ((xcor - meanX) ^ 2)]

  let sumFitness 0
  ask dogs [
    let sheepNum length influencedSheep
    if length influencedSheep = 0 [
      set sheepNum 1
    ]
    set points (sum map [x -> [newVariance] of x - [prevVariance] of x ] influencedSheep)/ sheepNum]
  ...
  [
    ...
    [
      ...
      [
        ...
        if rank < (random-float 0.5 * population)
        [
...
to action
  ...
  ifelse patch-here = patch meanX meanY [
    ...
    ifelse count sheeps in-cone distanceFromMean 90 > 1 [
...
to-report count-occurrences [number pairs-list]
  let occurrences 0
  foreach pairs-list [ x ->
    let pair-item first x
    if pair-item = number [
...
to-report random-between [minBetween maxBetween]
  report minBetween + random (maxBetween - minBetween + 1)
end
